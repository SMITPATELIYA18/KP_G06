...
/**
 * Handles sending/receiving data/messages to/from clients. Acts as a supervisor
 * to all other actors - creates ana manages them
 * 
...
public class SupervisorActor extends AbstractActor {
	...
	private ActorRef topicActor = null;
	 ...
	 * @param gitHubAPIInst Instance of <code>GitHubAPI</code> interface, for making
	 *                      external API calls to GitHub
	...
	public Receive createReceive() {
		return receiveBuilder().match(JsonNode.class, this::processRequest)
				.match(Messages.SearchResult.class, searchResult -> wsOut.tell(searchResult.searchResult, self()))
				.match(Messages.UserProfileInfo.class,
						userProfileInfo -> wsOut.tell(userProfileInfo.userProfileResult, self()))
				.match(Messages.RepositoryProfileInfo.class,
						repositoryProfileInfo -> wsOut.tell(repositoryProfileInfo.repositoryProfileResult, self()))
				.match(Messages.TopicInfo.class, topicInfo -> wsOut.tell(topicInfo.topicResult, self()))
	...
	/**
	 * Processes JSON data received from client requests, and creates actors to
	...
	private void processRequest(JsonNode receivedJson) {
		...
		if (receivedJson.has("search_query")) {
			 ...
			 * search actor created to handle this query. If no search actor is created,
			 * starts a new search actor and sends it a message to start tracking this
			 * search query. A new actor is created for every new search query and added to
			...
			/*
			 * For "user_profile" requests, checks if a "UserProfileActor" is already
			 * created. If no actor exists, creates a new "UserProfileActor" to handle all
			 * "user_profile" requests and sends it the current request to get the user
			 * profile information.
			 */
			String username = receivedJson.get("user_profile").asText();
			if (userProfileActor == null) {
				log.info("Creating a user profile actor.");
				userProfileActor = getContext()
						.actorOf(UserProfileActor.props(self(), this.gitHubAPIInst, this.asyncCacheApi));
			}
			userProfileActor.tell(new Messages.GetUserProfile(username), getSelf());
		} else if (receivedJson.has("repository_profile")) {

			/*
			 * For "repository_profile" requests, checks if a "RepositoryProfileActor" is
			 * already created. If no actor exists, creates a new "RepositoryProfileActor"
			 * to handle all "repository_profile" requests and sends it the current request
			...
			if (repositoryProfileActor == null) {
				...
				repositoryProfileActor = getContext()
						.actorOf(RepositoryProfileActor.props(self(), this.gitHubAPIInst, this.asyncCacheApi));
			}
			repositoryProfileActor.tell(new Messages.GetRepositoryProfileActor(username, repositoryName), getSelf());
		} else if (receivedJson.has("topic")) {

			String topic = receivedJson.get("topic").asText();
			if (topicActor == null) {
				log.info("Creating a topic actor.");
				topicActor = getContext().actorOf(TopicActor.props(self(), this.gitHubAPIInst, this.asyncCacheApi));
			}
			topicActor.tell(new Messages.GetTopic(topic), getSelf());
