...
public class TopicsTest {
	...
	public static void setUp() {
		...
		server = Server.forRouter((components) -> RoutingDsl.fromComponents(components).GET(routePattern)
				.routingTo((request) -> ok().sendResource(testResourceName)).build());
...
//
//		Http.RequestBuilder request = new Http.RequestBuilder().method(GET).uri(sampleTopicRepositoryListURL);
...
//	}
	...
	/**
	 * Checks if NOT_FOUND (404) status is returned when the request is not a GET
	 * request
	 * 
	 * @author Indraneel Rachakonda
	 */
...
//
//		Http.RequestBuilder request = new Http.RequestBuilder().method(POST).uri(sampleRepositoryProfileURL);
//		Result result = route(testApp, request);
//
...
//	}
	...
	public void should_ReturnTopicRepositoryList_provided_Topic() throws Exception {
		...
		assertEquals(Arrays.asList("goofing", "play", "test"),
				testRepositoryProfile.getRepositoryList().get(0).getTopics());
	}

	/**
	 * Checks if empty repository list is returned when no repositories are
	 * available for a topic name
	 * 
	 * @throws Exception In case of an error
	 * @author Indraneel Rachakonda
	 */
...
//	// UI
//	/**
//	 * Checks if repository list is displayed for provided topic via
//	 * topics.scala.html
//	 * 
//	 * @throws Exception In case of an error
//	 * @author Indraneel Rachakonda
...
//		JsonNode topicRepositoryList = mapper
//				.readTree(new File("test/resources/topicfeature/validTopicRepositoryList.json"));
//
//		Http.Request request = mock(Http.Request.class);
//		Content html = views.html.topics.topics.render(request,
//				new SearchRepository(topicRepositoryList, "sampleTopic"), assetsFinder);
...
//
//		Http.Request request = mock(Http.Request.class);
//		Content html = views.html.topics.topicsDisplay.render(request,
//				new SearchRepository(topicRepositoryList, "sampleTopic"), "java");
//		assertEquals("text/html", html.contentType());
//		assertTrue(contentAsString(html).contains("iamstillal"));
//	}
//
//	/**
//	 * Checks if "No Results found" is displayed when no repositories are available
//	 * for a topic name via topics.scala.html
//	 * 
//	 * @throws Exception In case of an error
//	 * @author Indraneel Rachakonda
//	 */
...
//		ObjectMapper mapper = new ObjectMapper();
//		JsonNode topicRepositoryList = mapper
//				.readTree(new File("test/resources/topicfeature/invalidTopicRepositoryList.json"));
//
//		Http.Request request = mock(Http.Request.class);
//		Content html = views.html.topics.topics.render(request,
//				new SearchRepository(topicRepositoryList, "sampleTopic"), assetsFinder);
...
//	/**
//	 * Checks if "No Results found" is displayed when no repositories are available
//	 * for a topic name via topicsDisplay.scala.html
//	 * 
//	 * @throws Exception In case of an error
//	 * @author Indraneel Rachakonda
//	 */
